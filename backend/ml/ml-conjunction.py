    ...
    try:
        ...
        def train_ml_ann(DATA, LABELS, LAYERS, iterations, TEST):
            ...
            for i in range(0, iterations):
                ...
                predict, AL, parameters = helpers.layers(DATA, LABELS, parameters, 1.75)
        ...
        print("Conjunction test set 1: " + result + " from " + str(DATA).replace("\n", "") + " passed: " + str(result == expected))

        DATA = np.array([[0, 1], [1, 1], [1, 1], [0, 1]])
        testResult = helpers.predict_ann(DATA.T, MODEL)
        expected = "[[False  True  True False]]"
        result = str(testResult)
        print("Conjunction test set 2: " + result + " from " + str(DATA).replace("\n", "") + " passed: " + str(result == expected))

        DATA = np.array([[1, 1], [1, 1], [1, 1], [1, 1]])
        testResult = helpers.predict_ann(DATA.T, MODEL)
        expected = "[[ True  True  True  True]]"
        result = str(testResult)
        print("Conjunction test set 3: " + result + " from " + str(DATA).replace("\n", "") + " passed: " + str(result == expected))

        DATA = np.array([[0, 1], [0, 1], [0, 1], [0, 1]])
        testResult = helpers.predict_ann(DATA.T, MODEL)
        expected = "[[False False False False]]"
        result = str(testResult)
        print("Conjunction test set 4: " + result + " from " + str(DATA).replace("\n", "") + " passed: " + str(result == expected))
